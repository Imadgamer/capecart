...
const SellerChat = () => {
  ...
  const { messages, loading: msgLoading } = useMessages(selectedConv);
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-3.5rem)]">
        <div className="px-4 py-3 border-b border-border flex items-center gap-3 bg-card">
          ...
          <p className="font-semibold text-sm">{conversations.find(c => c.id === selectedConv)?.other_user?.display_name || "Customer"}</p>
        </div>
          ...
          {conversations.map(conv => (
            <Card key={conv.id} className="p-3 flex items-center gap-3 cursor-pointer hover:bg-secondary/50 transition-colors" onClick={() => setSelectedConv(conv.id)}>
              <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center shrink-0">
                <span className="text-sm font-bold text-accent">{(conv.other_user?.display_name || "C")[0].toUpperCase()}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">{conv.other_user?.display_name || "Customer"}</p>
