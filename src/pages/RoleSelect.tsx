    ...
    <div className="min-h-screen bg-background flex flex-col">
      <div className="pt-16 pb-8 px-6 text-center">
        <div className="inline-flex items-center gap-3 mb-6">
          <img src={logo} alt="CapeCart" className="w-12 h-12 rounded-xl" />
          ...
          <div className="relative">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {["Browse Products", "Great Deals", "Chat with Seller", "Track Orders"].map(tag => (
                <span key={tag} className="text-xs px-2.5 py-1 rounded-full bg-accent/10 text-accent font-medium">{tag}</span>
              ))}
            </div>
          </div>
        </button>
